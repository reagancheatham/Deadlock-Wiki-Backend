  ...
  $(".mw-parser-output table.wikitable").each((_, table) => {
    ...
    $(table)
      ...
      .each((rowIndex, tr) => {
        ...
        const context =
          contextIndex >= 0 ? cleanText($(tds[contextIndex]).text()) : null;
    ...
    await Promise.all(
      urls.map(async (url) => {
        try {
          ...
          const guestNames = [
            ...new Set(quotes.map((q) => q.guestCharacter).filter(Boolean)),
          ];
          ...
          console.log(`Quotes imported successfully for ${mainCharacter}.`);
        } catch (err) {
